...
//  ------------------------------------------------------------
//  Simple vector QR-code generator for Adobe Illustrator (English UI).
...
(function () {
    ...
    // ------------------------------------------------------------------
    // 2. STRINGS  (English-only panel, left-to-right)
    // ------------------------------------------------------------------

    var L = {
        title: "QR2Vector",
        subtitle1: "Simple QRcode2Vector Generator by TalNadav",
        subtitle2: "tal74design@gmail.com",
        urlLabel: "1: Enter URL",
        sizeSectionLabel: "2: QR-Vector Size",
        sizeSmall: "SML",
        sizeMedium: "Md",
        sizeLarge: "Lrg",
        optionsSectionLabel: "3: Options",
        whiteBgLabel: "Add white background",
        blankCircleLabel: "Add empty space for custom logo",
        generateBtn: "make my day!",
        cancelBtn: "cancel",
        errNoDoc: "Please open an Illustrator document first.",
        errNoUrl: "Please enter a URL or some text.",
        errGeneric: "An error occurred while creating the QR code:\n",
    ...
    // ------------------------------------------------------------------
    // 3. UI  (English, left-to-right panel)
    // ------------------------------------------------------------------

    var g_finalAction = null;   // "generate" | "cancel" | null
    var g_finalData = null;

    // Panel geometry. The button row is split 1/3 (cancel) : 2/3 (make my day!)
    // across the full content width of the panel.
    var UI_WIDTH = 310;
    var BTN_GAP = 10;
    var CANCEL_W = Math.round((UI_WIDTH - BTN_GAP) / 3);      // 100
    var GENERATE_W = UI_WIDTH - BTN_GAP - CANCEL_W;            // 200
    ...
    function buildAndShowDialog() {
        ...
        urlLabel.preferredSize.width = UI_WIDTH;

        var urlInput = win.add("edittext", undefined, "https://");
        urlInput.preferredSize.width = UI_WIDTH;
        ...
        function addSizeToggle(parent, labelText, sizeIdx) {
            ...
            return { icon: icon, index: sizeIdx };
        }
        ...
        // Apply the default visual state ("Lrg" selected)
        refreshSizeToggles();

        // ---- 3: Options (independent square toggles) --------------------------
        var optionsLabel = win.add("statictext", undefined, L.optionsSectionLabel);
        optionsLabel.justify = align;
        optionsLabel.preferredSize.width = UI_WIDTH;
        ...
        function addOptionToggle(parent, labelText) {
            ...
            var lbl = g.add("statictext", undefined, labelText);
            lbl.justify = align;
            lbl.preferredSize.width = UI_WIDTH - 36;
